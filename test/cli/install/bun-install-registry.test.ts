...
describe("hoisting", async () => {
  ...
  describe("peers", async () => {
    ...
    for (const { dependencies, expected, situation } of peerTests) {
      // win-arm flake: the 3-round install loop intermittently sees a cached
      ...
      // on win-arm shard 3/8 specifically — x64 evidence absent, so scope
      // narrowly. Tracked separately from #29585.
      test.todoIf(
        (isFlaky && isMacOS && situation === "peer ^1.0.2") || (isFlaky && isWindows && process.arch === "arm64"),
      )(`it should hoist ${expected} when ${situation}`, async () => {
        await writeFile(
          packageJson,
          JSON.stringify({
            name: "foo",
